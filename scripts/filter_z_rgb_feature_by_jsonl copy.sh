...
OUT_ROOT="/datasets/ssv2_libero_90/stage2_rgb_libero90"

KEEP_REGEX="^libero_90_"
...
python filter_z_rgb_feature_by_jsonl.py \
  ...
  --part_size 8192 \
  > logs/filter_z_rgb_libero90_shard0.log 2>&1
...
python filter_z_rgb_feature_by_jsonl.py \
  ...
  --keep_regex "${KEEP_REGEX}" \
  --part_size 8192 \
  > logs/filter_z_rgb_libero90_shard1.log 2>&1
...
python filter_z_rgb_feature_by_jsonl.py \
  ...
  --keep_regex "${KEEP_REGEX}" \
  --part_size 8192 \
  > logs/filter_z_rgb_libero90_shard2.log 2>&1
...
python filter_z_rgb_feature_by_jsonl.py \
  ...
  --keep_regex "${KEEP_REGEX}" \
  --part_size 8192 \
  > logs/filter_z_rgb_libero90_shard3.log 2>&1

echo "Done filtering RGB features to LIBERO90."
