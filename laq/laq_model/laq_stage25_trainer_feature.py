...
Purpose:
    Train a geometry-aware latent refinement module:
        ...
        depth1 + z_rgb_features -> z_refined_logits
...
class LAQStage25Trainer(nn.Module):
    ...
    ----------
    model:
        LatentActionQuantizationStage25 model that accepts z_rgb_features.
    dataset:
        Dataset returning depth1, z_rgb_features, z_depth_indices.
    num_train_steps:
    ...
    def save_debug_input_batch(self, batch, depth1, z_rgb_features, z_depth_indices):
        if not self.is_main:
            return
        ...
        meta = []
        for i in range(n):
            z_rgb_feat = z_rgb_features[i].detach().cpu()

            item = {
                "z_rgb_features_shape": list(z_rgb_feat.shape),
                "z_rgb_features_mean": float(z_rgb_feat.float().mean()),
                "z_rgb_features_std": float(z_rgb_feat.float().std()),
    ...
    def _move_batch_to_device(self, batch):
        ...
        """
        required_keys = ["depth1", "z_rgb_features", "z_depth_indices"]
        ...
        return depth1, z_rgb_features, z_depth_indices
    ...
    def train_step(self):
        ...
        for _ in range(self.grad_accum_every):
            ...
            depth1, z_rgb_features, z_depth_indices = self._move_batch_to_device(batch)
            self.save_debug_input_batch(batch, depth1, z_rgb_features, z_depth_indices)
