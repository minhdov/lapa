"""
LAQ Stage 2.5 Model 5 Trainer

Purpose:
    Train a latent feature predictor:

        z_rgb_features -> pred_z_depth_feature
...
class LAQStage25TrainerModel5(nn.Module):
    """
    Trainer for Model 5:
    ...
    No depth1, no z_depth_indices, no token accuracy, no CE loss.
    """
    ...
    def __init__(
        ...
        *,
        ...
        results_folder: str = "./model5_stage25_feature_distill_results",
        use_wandb: bool = True,
        wandb_project: str = "lapa_depth_model5",
        ...
        self.save_best_every = int(save_best_every)
    ...
    def save_debug_input_batch(self, batch, z_rgb_features, z_depth_feature):
        if not self.is_main:
            return
    ...
    def _move_batch_to_device(self, batch):
        required_keys = ["z_rgb_features", "z_depth_feature"]
        ...
        return z_rgb_features, z_depth_feature
    ...
    def _maybe_save_best(self, logs: Dict[str, Any]):
        ...
        step = int(logs["step"])
        loss = float(logs["model5/loss"])
        mse = float(logs["model5/feature_mse_loss"])
        cosine = float(logs["model5/feature_cosine_loss"])
        ...
        best_path = self.results_folder / "model5.best.pt"
        self.save(
            best_path,
            extra={
        ...
        with open(self.results_folder / "model5.best.json", "w", encoding="utf-8") as f:
    ...
    def train_step(self):
        ...
        for _ in range(self.grad_accum_every):
            ...
            z_rgb_features, z_depth_feature = self._move_batch_to_device(batch)

            self.save_debug_input_batch(batch, z_rgb_features, z_depth_feature)
        ...
        logs = {
            "model5/loss": total_loss,
            "model5/feature_mse_loss": total_mse,
            "model5/feature_cosine_loss": total_cosine,
            "model5/lr": self.lr,
            "model5/best_loss": self.best_loss,
            "model5/best_mse": self.best_mse,
            "model5/best_cosine": self.best_cosine,
            "model5/best_step": self.best_step,
            "step": steps,
        }

        if self.save_best and self.save_best_every > 0 and steps % self.save_best_every == 0:
        ...
        if self.is_main and self.save_model_every > 0 and steps % self.save_model_every == 0:
            ckpt_path = self.results_folder / f"model5.{steps}.pt"
            self.save(ckpt_path)
    ...
    def train(self, log_fn: Callable = noop):
        ...
        while int(self.steps.item()) < self.num_train_steps:
            ...
            if self.is_main and int(self.steps.item()) % self.log_every == 0:
                ...
                self.print(
                    ...
                    f"loss {logs['model5/loss']:.6f} | "
                    f"mse {logs['model5/feature_mse_loss']:.6f} | "
                    f"cosine {logs['model5/feature_cosine_loss']:.6f} | "
        ...
        if self.save_final and self.is_main:
            ...
            final_ckpt_path = self.results_folder / f"model5.{final_step}.pt"
            self.save(
                final_ckpt_path,
                extra={
        ...
        self.print("Model 5 training complete")
