...
def flush_feature_buffer(
    ...
    pkg = {
        ...
        "z_rgb_indices_input": z_rgb_indices_input,
    ...
    part_info = {
        ...
        "path": str(out_path),
        "num_samples": n,
        "z_rgb_indices_input_shape": list(z_rgb_indices_input.shape),
...
def run_inference(args):
            ...
            for batch_idx, batch in enumerate(loader):
                ...
                for i in range(batch_size):
                    ...
                    item = {
                        ...
                        "z_rgb_indices_input": tensor_to_list(z_rgb_cpu[i]),
                    ...
                    feature_buffer["z_rgb_indices_input"].append(z_rgb_cpu[i])
    ...
    manifest = {
        ...
        "model_definition": (
            "Model1 / Stage 2.5: depth image + z_rgb_indices_input "
        ...
        "feature_definition": {
            "z_rgb_indices_input": (
                "Input RGB latent tokens from pretrained LAPA / Stage 2."
            ),
            "z_depth_indices_gt": (
                "Ground-truth Stage 1 depth latent tokens from z_depth_path."
            ),
            "z_depth_indices_pred": (
                "Predicted depth latent tokens from logits.argmax(dim=-1)."
            ),
            "z_depth_feature_pred": (
                "Predicted/refined depth feature output from Stage 2.5 Model 1."
            ),
            "confidence": (
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Inference/evaluation for LAPA-depth Stage 2.5 Model 1, "
            "saving standardized JSONL + .pt feature parts."
        )
    )
    ...
    parser.add_argument(
        "--save_feature_jsonl",
        action="store_true",
        help="Save full z_depth_feature_pred into JSONL. Not recommended except debugging.",
    )
