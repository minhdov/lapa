...
class FeaturePartWriter:
    ...
    def save_manifest(self, args, metrics=None):
        manifest = {
            ...
            "model_definition": (
                "Model7.1 / Stage 2.5: depth1 + z_rgb_feature_input "
                "-> z_depth_feature_pred"
            ),
            ...
            "feature_definition": {
                ...
                "z_depth_feature_pred": (
                    "Predicted depth feature from Model7.1 extract_z_depth_feature(depth1, z_rgb_feature_input)."
                ),
                "z_depth_feature_gt": (
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Model7.1 inference: depth1 + z_rgb_feature_input "
            "-> z_depth_feature_pred"
        )
    )
    ...
    parser.add_argument("--feature_prefix", type=str, default="z_depth_val_model7_1")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader)):
            ...
            if args.compute_metrics:
                ...
                gt_flat = gt_z_depth_feature.reshape(
                    gt_z_depth_feature.shape[0],
                    -1,
                )
            ...
            batch_ids = batch.get(
                "id",
                [str(i) for i in range(z_depth_feature_pred_cpu.shape[0])],
            )
            batch_depth_paths = batch.get(
                "depth1_path",
                [""] * z_depth_feature_pred_cpu.shape[0],
            )
