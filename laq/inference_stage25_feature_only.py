...
from stage252_dataset import Stage252Dataset
from latent_action_quantization_stage252 import LatentActionQuantizationStage252
    ...
    with out_path.open("w", encoding="utf-8") as fout:
        with torch.no_grad():
            for batch_idx, batch in enumerate(loader):
                ...
                batch_size = z_rgb_features.shape[0]
...
def main():
    parser = argparse.ArgumentParser(
        description="Inference/evaluation for LAPA-depth Stage 2.5.2 RGB-feature-only model."
    )
    ...
    parser.add_argument(
        "--output_jsonl",
        type=str,
        default="stage252_predictions.jsonl",
    )
