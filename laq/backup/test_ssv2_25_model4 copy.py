...
class FeaturePartWriter:
    def __init__(self, output_dir: str, prefix: str, part_size: int):
        ...
        self.buffer = {
            "id": [],
            "depth1_path": [],
            "z_depth_feature": [],      # Model4 output feature
            "gt_z_depth_feature": [],   # optional
            "z_rgb_features": [],       # optional
            "z_depth_indices": [],      # optional, if dataset returns it
            "action_t": [],             # optional, if dataset returns it
        }
    ...
    def add(
        ...
        if gt_z_depth_feature is not None:
            self.buffer["gt_z_depth_feature"].append(gt_z_depth_feature.detach().cpu().float())

        if z_rgb_features is not None:
            self.buffer["z_rgb_features"].append(z_rgb_features.detach().cpu().float())

        if z_depth_indices is not None:
            self.buffer["z_depth_indices"].append(z_depth_indices.detach().cpu().long())

        if action_t is not None:
            self.buffer["action_t"].append(action_t.detach().cpu().float())
    ...
    def flush(self, force=False):
        ...
        if len(self.buffer["gt_z_depth_feature"]) == n:
            gt_z_depth_feature = torch.stack(self.buffer["gt_z_depth_feature"], dim=0).float()
            pkg["gt_z_depth_feature"] = gt_z_depth_feature
            part_info["gt_z_depth_feature_shape"] = list(gt_z_depth_feature.shape)

        if len(self.buffer["z_rgb_features"]) == n:
            z_rgb_features = torch.stack(self.buffer["z_rgb_features"], dim=0).float()
            pkg["z_rgb_features"] = z_rgb_features
            part_info["z_rgb_features_shape"] = list(z_rgb_features.shape)

        if len(self.buffer["z_depth_indices"]) == n:
            z_depth_indices = torch.stack(self.buffer["z_depth_indices"], dim=0).long()
            pkg["z_depth_indices"] = z_depth_indices
            part_info["z_depth_indices_shape"] = list(z_depth_indices.shape)

        if len(self.buffer["action_t"]) == n:
            action_t = torch.stack(self.buffer["action_t"], dim=0).float()
    ...
    def save_manifest(self, args, metrics=None):
        manifest = {
            ...
            "feature_definition": {
                "z_depth_feature": "Model4 output feature from extract_z_depth_feature(depth1, z_rgb_features)",
                "gt_z_depth_feature": "Ground-truth z_depth_feature from Stage 1 or previous feature manifest, saved only if --save_gt is used",
                "z_rgb_features": "Input RGB feature, saved only if --save_rgb is used",
                "z_depth_indices": "Depth latent action token, saved only if dataset returns z_depth_indices",
                "action_t": "LIBERO 7-DoF action vector, saved only if dataset returns action_t",
            },
            "parts": self.parts,
        }
...
def main():
    parser = argparse.ArgumentParser(
        description="Model4 inference: depth1 + z_rgb_features -> z_depth_feature"
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader)):
            ...
            z_depth_feature = model.extract_z_depth_feature(
                depth1=depth1,
                z_rgb_features=z_rgb_features,
            )

            if z_depth_feature.shape != gt_z_depth_feature.shape:
                raise RuntimeError(
                    f"Prediction and GT shape mismatch: "
                    f"pred={tuple(z_depth_feature.shape)}, "
            ...
            if args.compute_metrics:
                ...
                pred_flat = z_depth_feature.reshape(z_depth_feature.shape[0], -1)
                ...
                bs = z_depth_feature.shape[0]
                total_mse += float(mse.sum().detach().cpu())
                total_cosine_loss += float(cosine_loss.sum().detach().cpu())
                total_metric_samples += bs

            z_depth_feature_cpu = tensor_to_cpu(z_depth_feature)
            gt_z_depth_feature_cpu = tensor_to_cpu(batch["z_depth_feature"]) if args.save_gt else None
            z_rgb_features_cpu = tensor_to_cpu(batch["z_rgb_features"]) if args.save_rgb else None

            z_depth_indices_cpu = None
            if "z_depth_indices" in batch:
                z_depth_indices_cpu = tensor_to_cpu(batch["z_depth_indices"])

            action_t_cpu = None
            if "action_t" in batch:
                action_t_cpu = tensor_to_cpu(batch["action_t"])

            batch_ids = batch.get("id", [str(i) for i in range(z_depth_feature_cpu.shape[0])])
            batch_depth_paths = batch.get("depth1_path", [""] * z_depth_feature_cpu.shape[0])

            bs = z_depth_feature_cpu.shape[0]
            ...
            for i in range(bs):
                ...
                feature_writer.add(
                    ...
                    z_depth_feature=z_depth_feature_cpu[i],
                    gt_z_depth_feature=gt_z_depth_feature_cpu[i] if gt_z_depth_feature_cpu is not None else None,
                    z_rgb_features=z_rgb_features_cpu[i] if z_rgb_features_cpu is not None else None,
                    z_depth_indices=z_depth_indices_cpu[i] if z_depth_indices_cpu is not None else None,
                    action_t=action_t_cpu[i] if action_t_cpu is not None else None,
                )
                ...
                if output_jsonl_f is not None:
                    item = {
                        "id": sample_id,
                        "depth1_path": depth1_path,
                        "z_depth_feature_shape": list(z_depth_feature_cpu[i].shape),
                    }
            ...
            if batch_idx % args.log_every == 0:
                print(
                    ...
                    f"model4_feature_shape {list(z_depth_feature.shape)}"
                )
